...
async def init_db() -> None:
    async with engine.begin() as conn:
        ...
        await conn.run_sync(_ensure_expense_recurring_group_column)
...
def _ensure_expense_recurring_group_column(sync_conn) -> None:
    inspector = inspect(sync_conn)
    table_names = set(inspector.get_table_names())
    if "expenses" not in table_names:
        return
    ...
    column_names = {column["name"] for column in inspector.get_columns("expenses")}
    if "recurring_group_id" not in column_names:
        dialect = sync_conn.dialect.name
        if dialect == "postgresql":
            sync_conn.exec_driver_sql(
            ...
            sync_conn.exec_driver_sql(
                "ALTER TABLE expenses ADD COLUMN recurring_group_id VARCHAR(36)"
            )

    index_names = {index.get("name", "") for index in inspector.get_indexes("expenses")}
    if "ix_expenses_recurring_group_id" not in index_names:
        sync_conn.exec_driver_sql(
            "CREATE INDEX IF NOT EXISTS ix_expenses_recurring_group_id "
            "ON expenses (recurring_group_id)"
        )
