...
class HouseholdSubcategory(SQLModel, table=True):
    __tablename__ = "household_subcategories"
    __table_args__ = (
        UniqueConstraint(
            "household_category_id",
            "normalized_name",
            name="uq_household_subcategory_normalized_name",
        ),
    )

    id: UUID = Field(default_factory=uuid4, primary_key=True, index=True)
    household_category_id: UUID = Field(
        foreign_key="household_categories.id",
        nullable=False,
        index=True,
    )
